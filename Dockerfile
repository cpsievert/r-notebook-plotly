...
RUN echo 'options(repos = c(CRAN = "https://cran.rstudio.com/"), warn = -1); zz <- file("messages.Rout", open = "wt"); sink(zz, type = "message")' >> ~/.Rprofile \
	...
	&& Rscript -e "update.packages(ask=F); install.packages('ggthemes'); install.packages('DT'); devtools::install_github('hadley/ggplot2'); devtools::install_github('ropensci/plotly')"
